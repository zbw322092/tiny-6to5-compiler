 ...
 * reference url: 
 * 1. https://mathiasbynens.be/notes/javascript-encoding#surrogate-formulae
 * 2. https://www.ecma-international.org/ecma-262/8.0/index.html#sec-utf16encoding
 * 
 * @param cp 
 */
export const fromCodePoint = (cp: number): string => {
  if (cp >= 0x0000 && cp <= 0xFFFF) {
    return String.fromCharCode(cp);
  } else {
    let high = Math.floor((cp - 0x10000) / 0x400) + 0xD800;
    let low = (cp - 0x10000) % 0x400 + 0xDC00;
 ...
 * @param cp 
 */
export const isWhiteSpace = (cp: number): boolean => {
 ...
 * @param cp
 */
export const isIdentifierStart = (cp: number): boolean => {
 ...
 * @param cp
 */
export const isIdentifierPart = (cp: number): boolean => {
  return (cp === 0x24) || (cp === 0x5F) ||  // $ (dollar) and _ (underscore)
    (cp >= 0x41 && cp <= 0x5A) ||         // A..Z
    (cp >= 0x61 && cp <= 0x7A) ||         // a..z
    (cp >= 0x30 && cp <= 0x39) ||         // 0..9
